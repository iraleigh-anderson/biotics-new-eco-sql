...
example$elem.glo.sql <- paste0(
  "INSERT INTO element_global (ELCODE_BCD, GNAME_ID, CONCEPT_REFERENCE_ID, CONCEPT_NAME_ID, G_PRIMARY_COMMON_NAME, D_CLASSIFICATION_STATUS_ID, D_LANGUAGE_ID, REC_CREATE_USER, D_MAINTAINED_BY_STATUS_ID) 
  VALUES ('",
...
sql_filename <- paste0("Ecology_Element_Insert_", today, ".sql")
...
BGCexample <- data.frame(
  ...
  bgc = c("ICHxw", "MSdw", "ESSFxv", "BGxh1", "PPdh2"),
  ss = c("Wm05","01","02","11","Wm05")
  )

# 13. INSERT Statements for BC_COMM_SUB_BGC 
...
# Create the filename with the date appended
bgc.sql.name <- paste0("Ecology_BGC_Insert_", today, ".sql")
...
# Create the filename with the date appended
#bgc.csv.name <- paste0("Ecology_BGC_Insert_", today, ".csv")
# Write the CSV file
#write.csv(BGCexample, file = bgc.csv.name, row.names = FALSE)
